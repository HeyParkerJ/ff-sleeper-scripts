...
app.get(
  '/api/matchups/csv/:startingSeason/:endingSeason/:startingWeek/:endingWeek',
  async (req, res) => {
    const {
      startingSeason,
      endingSeason,
    ...
    const seasonsToRetrieve = getSeasonsToRetrieve(
      startingSeason,
      endingSeason,
      seasonToLeagueIdKVP,
    );
    ...
    const leagueID = transformSeasonToLeagueId(season);
    const data = await fetchData(useHttp, leagueID, false);
    const result = doCSVCalculation(data, season);
  },
);
...
const seasonToLeagueIdKVP = {
  2019: '473636235902971904',
  2020: '603631612793520128',
  2021: '687728692536893440',
  2022: '852771702776672256',
};
...
const transformSeasonToLeagueId = (season) => {
  switch (season) {
    ...
    default:
      throw new Error(
        'unsupported season supplied to transformSeasonToLeagueID',
      );
  }
};
